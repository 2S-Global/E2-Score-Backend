...
export const getResume = async (req, res) => {
  try {
    ...
    // For Language
    const languageIds = getUniqueIds(userDetails.languageProficiency, 'language');
    // For Language proficiency
    const languageProficiencyIds = getUniqueIds(userDetails.languageProficiency, 'proficiency');
    ...
    const [universities, institutes, courses, boards, levels, courseTypes, gradingSystemName, companies, socialProfiles, skills, itSkillNameList, taggedWithNames, currentIndustry, currentDepartment, jobRole, locations, languageName, proficiencyName, workPermitOtherName, categoryName, disabilityTypeName, breakReasonName, maritalStatusName, usaPermitName, addiInfoName, userGender] = await Promise.all([
      ...
      // For Online Profiles
      list_social_profile.find({ _id: { $in: socialProfileIds }, is_del: 0 }).lean(),
      // skill name value from personal details
      list_key_skill.find({ _id: { $in: userDetails.skills || [] } }).lean(),
      // Getting IT Skill Name
      list_tech_skill.find({ _id: { $in: itSkillIds } }).select('name').lean(),
      // Getting taggedwithName  from user projects
      list_project_tag.find({ _id: { $in: taggedWithIds } }).lean(),
      //Getting Current Industry
      list_industries.findOne({ id: userPref.CurrentIndustry }).select("job_industry").lean(),
      // Getting Current Department
      list_department.findOne({ id: userPref.CurrentDepartment }).select("job_department").lean(),
      //Getting Job Role
      list_job_role.findById(userPref.JobRole).select("job_role").lean(),
      // Getting Locations
      list_india_cities.find({ _id: { $in: userPref.location } }).select("city_name").lean(),
      // Getting Language Name
      list_language.find({ _id: { $in: languageIds } }).select("name").lean(),
      //Getting Language Proficiency
      list_language_proficiency.find({ _id: { $in: languageProficiencyIds } }).select("name").lean(),
      // Getting work permit other name
      list_tbl_countrie.find({ _id: { $in: userDetails.workPermitOther } }).select("name").lean(),
    ...
    const employment = employmentsRaw.map(job => ({
      ...job,
      companyName: companyMap[job.companyName.toString()] || 'Company Name Not Found',
    }));
    ...
    const userPersonalDetails = {
      ...
      workPermitOtherNames: (userDetails.workPermitOther || []).map(id => workPermitOtherNameWithMap[id] || ''),
